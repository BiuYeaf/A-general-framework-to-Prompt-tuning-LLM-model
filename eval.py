...
from transformers import AutoConfig, AutoModelForCausalLM,default_data_collator,AutoTokenizer
...
parser.add_argument('--testset_dir', type=str, default='/test_dir')
...
for idx, inputs in enumerate(test_dataloader):
    with torch.no_grad():
        outputs = accelerator.unwrap_model(model).generate(
            input_ids=inputs["input_ids"].to("cuda"), attention_mask=inputs["attention_mask"].to("cuda"), max_new_tokens=args.max_length
        )
...
precision,recall,f1,c_matrix = evaluate_score(labels,pred,'macro',label1_name,label2_name)
index = 1
for i in results:
    for j in i:
